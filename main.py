...
class RickiCommandCenter:
        ...
        try:
            ...
            if text:
                ...
                self.messages.append({
                    "role": "user",
                    "content": f"[Document Library: {fname}]\n{text}"
                })
    ...
    def scan_visuals(self):
        ...
        self.update_chat("SYSTEM", "Uploading visual data...")
    ...
    def popout_operative(self):
        if self.popout_window is None or not self.popout_window.winfo_exists():
            ...
            def on_move(e):
                nx = self.popout_window.winfo_x() - self._x + e.x
                ny = self.popout_window.winfo_y() - self._y + e.y
                self.popout_window.geometry(f"+{nx}+{ny}")

            self.float_label.bind("<ButtonPress-1>", start_move)
            self.float_label.bind("<B1-Motion>", on_move)

            self.float_label.bind(
                "<Button-3>",
                lambda e: [
                    self.popout_window.destroy(),
                    self.avatar_dock.pack(
                        side="top",
                        fill="x",
                        pady=(20, 10),
                        before=self.file_list_frame
                    )
                ]
            )
